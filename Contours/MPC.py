...
SN = np.loadtxt('Cobaya_Chains/chains/SN/%s_SN.1.txt' %(model), usecols=(cols), comments='#')
...
BAO_CMB = np.loadtxt('Cobaya_Chains/chains/CMB+BAO/%s_CMB_BAO_speed3.1.txt' %(model), usecols=(cols), comments='#')
BAO_CMB_weights = np.loadtxt('Cobaya_Chains/chains/CMB+BAO/%s_CMB_BAO_speed3.1.txt' %(model), usecols=(0), comments='#')
...
c.add_chain(SN_2, parameters=label, weights=SN_2_weights, linewidth=1.0, name="SN_22", kde=1.5, linestyle = '--', color="red",num_free_params=len(begin))
c.configure(summary=True, shade_alpha=1, shade=[True, False, True, True, False, False],statistics="max")
...
# Best Fit Parameters
# SN Only
p0_sn = c.analysis.get_summary(chains="SN_2")[label[0]][1]
p0p_sn = c.analysis.get_summary(chains="SN_2")[label[0]][2]-c.analysis.get_summary(chains="SN_2")[label[0]][1]
p0m_sn = c.analysis.get_summary(chains="SN_2")[label[0]][1]-c.analysis.get_summary(chains="SN_2")[label[0]][0]
p1_sn = c.analysis.get_summary(chains="SN_2")[label[1]][1]
p1p_sn =c.analysis.get_summary(chains="SN_2")[label[1]][2]-c.analysis.get_summary(chains="SN_2")[label[1]][1]
p1m_sn =c.analysis.get_summary(chains="SN_2")[label[1]][1]-c.analysis.get_summary(chains="SN_2")[label[1]][0]
...
ax.vlines(1, -5, 5, colors='k', linestyles='--', alpha=0.9)
