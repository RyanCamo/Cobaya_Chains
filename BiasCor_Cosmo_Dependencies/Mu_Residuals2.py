...
norm_bias_bin = np.genfromtxt("/Users/RyanCamo/Downloads/PIPPIN_OUTPUT/Rep2/hubble_diagram_BIN_NORM.txt", names=True)
mu_norm_bin = norm_bias_bin['MU']
zz_norm_bin = norm_bias_bin['zCMB']

norm_bias = np.genfromtxt("/Users/RyanCamo/Downloads/PIPPIN_OUTPUT/Rep2/hubble_diagram_UNBIN_NORM.txt", names=True)
mu_norm = norm_bias['MU']
zz_norm = norm_bias['zCMB']


# Pippin output after Bias corrections with Om = 0.5 Ol = 0.5.
mod_bias_bin = np.genfromtxt("/Users/RyanCamo/Downloads/PIPPIN_OUTPUT/Rep2/hubble_diagram_BIN_MOD.txt", names=True)
mu_mod_bin = mod_bias_bin['MU']
zz_mod_bin = mod_bias_bin['zCMB']
mod_bias = np.genfromtxt("/Users/RyanCamo/Downloads/PIPPIN_OUTPUT/Rep2/hubble_diagram_UNBIN_MOD.txt", names=True)
mu_mod = mod_bias['MU']
...
axs.scatter(zz_norm_bin, mu_mod_bin-mu_norm_bin, color = 'k', s=40, label = r'BIN: $\omega_{\mathrm{ref}}=-1.028$')
plt.legend(loc='lower left', frameon=True,fontsize=12)
axs.set_ylim(-0.1,0.06)
